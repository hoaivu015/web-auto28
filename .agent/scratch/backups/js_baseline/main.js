document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderLoadedVehicles() {
        ...
        // Handle URL query parameters for model filtering (e.g. index.html?filter=vf3#inventory)
        try {
            const urlParams = new URLSearchParams(window.location.search);
            const filterParam = urlParams.get('filter') || urlParams.get('model');
            if (filterParam) {
                const filterVal = filterParam.toLowerCase().trim();
                const targetPill = document.querySelector(`.filter-pill[data-filter="${filterVal}"]`);
                if (targetPill) {
                    document.querySelectorAll('.filter-pill').forEach(p => p.classList.remove('active'));
                    targetPill.classList.add('active');
                } else if (['lux-a', 'lux-sa', 'fadil', 'gas'].includes(filterVal)) {
                    const gasPill = document.querySelector('.filter-pill[data-filter="gas"]');
                    if (gasPill) {
                        document.querySelectorAll('.filter-pill').forEach(p => p.classList.remove('active'));
                        gasPill.classList.add('active');
                    }
                }
                filterCarCards(filterVal);
                ...
                if (inventorySec) {
                    setTimeout(() => {
                        inventorySec.scrollIntoView({ behavior: 'smooth', block: 'start' });
                    }, 350);
                }
            }
        } catch (err) {
            console.warn('[Auto28 Filter] URL parameter parse error:', err);
        }
    }
